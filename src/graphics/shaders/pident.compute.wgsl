// Compute shader for percentage identity colorscheme.
// Computes a bitmask for each column indicating consensus residue and identity threshold.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Determine what conservation threshold is reached
    var bucket = 0u;
    if (moreThanPercent(maxCount, nonGapCount, 80u)) {
        bucket = 3u;
    } else if (moreThanPercent(maxCount, nonGapCount, 60u)) {
        bucket = 2u;
    } else if (moreThanPercent(maxCount, nonGapCount, 40u)) {
        bucket = 1u;
    }
    
    // Consensus is residue that meets >50% threshold, otherwise 0
    // var consensusResidue = moreThanPercent(maxCount, nonGapCount, 50u) ? maxResidue : 0u;
    
    // Set bits in the mask 
    // First 8 bits for consensus residue, next 2 bits for conservation bucket (0-3)
    var mask = 0u;
    mask = mask | maxResidue;
    mask = mask | (bucket << 8u);
